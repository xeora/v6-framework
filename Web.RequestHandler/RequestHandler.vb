Namespace SolidDevelopment.Web
    Public Class RequestHandler
        ...
        Public Function BeginProcessRequest(ByVal context As System.Web.HttpContext, ByVal cb As System.AsyncCallback, ByVal extraData As Object) As System.IAsyncResult Implements System.Web.IHttpAsyncHandler.BeginProcessRequest
            Dim rH As New HandleRequestDelegate(AddressOf Me.HandleRequest)

            Dim asynch As New AsynchOperation(rH, cb, context, extraData)

            ' Prepare RequestDateTime
        ...
        Class AsynchOperation
            ...
            Private _requestHandler As HandleRequestDelegate
            ...
            Public Sub New(ByVal rH As HandleRequestDelegate, _
                            ByVal callback As AsyncCallback, _
                            ByVal context As System.Web.HttpContext, _
                            ByVal state As Object)

                Me._requestHandler = rH
            ...
            Public Sub StartAsyncWork()
                Me._requestHandler.BeginInvoke(Me._context, New AsyncCallback(AddressOf Me.EndAsyncWork), Nothing)
            End Sub

            Private Sub EndAsyncWork(ByVal aR As IAsyncResult)
                Try
                    Me._requestHandler.EndInvoke(aR)
                ...
                Me._completed = True
                Me._callback.Invoke(Me)
            End Sub
        End Class

        Public Delegate Sub HandleRequestDelegate(ByVal context As System.Web.HttpContext)
        Private Sub HandleRequest(ByVal context As System.Web.HttpContext)
            ...
            SolidDevelopment.Web.General.AssignRequestID(Me._RequestID)
            ' !--
...
RECHECKFORMAPPING:
                ...
                If URLMI.URLMapping Then
                    Dim ResolvedMapped As PGlobals.URLMappingInfos.URLMappingItem.ResolvedMapped = _
                        PGlobals.URLMappingInfos.URLMappingItem.ResolveMappedURL(General.Context.Request.RawUrl)
                    ...
                    If Not ResolvedMapped Is Nothing AndAlso _
                        ...
                        If ResolvedMapped.CachingType <> PGlobals.PageCachingTypes.AllContent Then
                            QueryString.AppendFormat("nocache={0}", _
                                 General.GetCachingTypeText( _
                                    ResolvedMapped.CachingType) _
                                )

                            If ResolvedMapped.QueryStrings.Count > 0 Then _
                                QueryString.Append("&")
                        End If
                    ...
                    Else ' this is not a mapped request (convert mapped if it's available)
                        If String.Compare(General.Context.Request.HttpMethod, "GET", True) = 0 Then
                            ' Reverse Mapping only available on GET Requests
                            Dim TemplateID As String = General.ResolveTemplateFromURL(General.Context.Request.RawUrl)

                            If Not TemplateID Is Nothing AndAlso _
                                TemplateID.IndexOf("_bi_sps_v") = -1 Then
                                ...
                                If Not rMItem Is Nothing AndAlso _
                                    Not String.IsNullOrEmpty(rMItem.ResolveInfo.MapFormat) Then

                                    Dim QueryString As New Generic.List(Of PGlobals.URLQueryInfo)
                                    Dim qSL As PGlobals.URLQueryInfo() = _
                                        PGlobals.URLQueryInfo.ResolveQueryItems( _
                                            General.Context.Request.ServerVariables.Item("QUERY_STRING") _
                                        )

                                    Dim Caching As PGlobals.PageCachingTypes
                                    For Each qS As PGlobals.URLQueryInfo In qSL
                                        If String.Compare(qS.ID, "nocache", True) = 0 Then
                                            Select Case qS.Value
                                                Case "L1"
                                                    Caching = PGlobals.PageCachingTypes.TextsOnly
                                                Case "L2"
                                                    Caching = PGlobals.PageCachingTypes.NoCache
                                                Case "L0XC"
                                                    Caching = PGlobals.PageCachingTypes.AllContentCookiless
                                                Case "L1XC"
                                                    Caching = PGlobals.PageCachingTypes.TextsOnlyCookiless
                                                Case "L2XC"
                                                    Caching = PGlobals.PageCachingTypes.NoCacheCookiless
                                                Case Else
                                                    Caching = PGlobals.PageCachingTypes.AllContent
                                            End Select
                                        Else
                                            QueryString.Add(qS)
                                        End If
                                    ...
                                    Dim RedirectLocation As String = _
                                        General.GetPageToRedirect(True, TemplateID, Caching, QueryString.ToArray())
                ...
                Dim AcceptsGZip As Boolean, AcceptEncodings As String = General.Context.Request.ServerVariables.Item("HTTP_ACCEPT_ENCODING")
                If Not AcceptEncodings Is Nothing Then AcceptsGZip = (AcceptEncodings.IndexOf("gzip") <> -1) Else AcceptsGZip = False

                ' Create Helper Control and Process Request
                '   Check Requested Path For Theme and Translation Seperation
                Dim RequestedTemplate As String = SolidDevelopment.Web.General.ResolveTemplateFromURL(General.Context.Request.FilePath)
                ...
                If RequestedTemplate Is Nothing Then
                    ...
                    Dim ThemeWebPath As String = General.GetThemePublicContentsPath(ThemeID, TranslationID)
                    Dim RequestFileVirtualPath As String = General.Context.Request.Path

                    If RequestFileVirtualPath.IndexOf(ThemeWebPath) = -1 Then
                        ' Exam Match Values
                        Me.FindRequestPathThemeAndTranslation(RequestFileVirtualPath, ThemeID, TranslationID)

                        ' If Not match any create with default ones
                        Me._ThemeWebControl = New Managers.ThemeWebControl(ThemeID, TranslationID)

                        ' Update ThemeWebPath variable with the new ones
                        ThemeWebPath = General.GetThemePublicContentsPath(ThemeID, TranslationID)
                    Else
                        ' Use the Default Settings
                        Me._ThemeWebControl = New Managers.ThemeWebControl(ThemeID, TranslationID)
                    End If

                    ' Set URLMapping Settings and reload page for correct mapping
                    Dim PreURLMappingExamString As String = _
                        String.Format("{0}_MappingInfo", Configurations.VirtualRoot.Replace("/"c, "_"c))
                    Dim MappingIsDefined As Boolean = _
                        Not General.Context.Application.Contents.Item(PreURLMappingExamString) Is Nothing
                    General.Context.Application.Contents.Item(PreURLMappingExamString) = Me._ThemeWebControl.URLMappingInfo

                    If Not MappingIsDefined Then GoTo RECHECKFORMAPPING
                    ' !--
                    ...
                    If RequestFileVirtualPath.IndexOf(ThemeWebPath) > -1 Then
                        ...
                        Dim PublicContentWebPath As String = _
                            IO.Path.GetDirectoryName(RequestFileVirtualPath)

                        Dim RequestedFilePath As String = _
                            IO.Path.GetFileName(RequestFileVirtualPath)

                        Dim RequestFileStream As IO.Stream = Nothing

                        ' Write File Content To Theme File Stream
                        Me._ThemeWebControl.ProvideFileStream(RequestFileStream, PublicContentWebPath, RequestedFilePath)
                        ...
                        Else
                            Me.WriteOutput( _
                                    General.GetMimeType( _
                                        IO.Path.GetExtension(RequestFileVirtualPath) _
                                    ), _
                                    RequestFileStream, _
                                    AcceptsGZip _
                                )
...
POSTFILESTREAM:
                        ...
                        Dim RequestFilePath As String = General.Context.Request.PhysicalPath
                    ...
                    End If
                Else
                    ' Create ThemeWebControl Object with Defaults
                    Me._ThemeWebControl = New Managers.ThemeWebControl(ThemeID, TranslationID)
                    ...
                    Else
                        ...
                        If Me._ThemeWebControl.ServiceID Is Nothing Then GoTo POSTFILESTREAM
                        ...
                        If String.Compare(General.Context.Request.HttpMethod, "GET", True) = 0 Then
                            ...
                            Dim mR_Parent As System.Text.RegularExpressions.Match = _
                                System.Text.RegularExpressions.Regex.Match(ParentURL, "hc-\d+")

                            If Not mR_Parent.Success Then
                                Dim RewrittenPath As String = _
                                    String.Format("{0}hc-{1},{2}", Configurations.ApplicationRoot.BrowserSystemImplementation, General.HashCode, RequestedTemplate)
                        ...
                        If Me._ThemeWebControl.IsAuthenticationRequired Then
                            Select Case Me._ThemeWebControl.ServiceType
                                Case Managers.Theme.SettingsClass.ServicesClass.ServiceItem.ServiceTypes.template
                                    ...
                                    If Not RequestedTemplate Is Nothing AndAlso _
                                        String.Compare(AuthenticationPage, RequestedTemplate, True) <> 0 Then

                                        SyncLock General.Context.Session.SyncRoot
                                            Try
                                                SolidDevelopment.Web.General.Context.Session.Contents.Item("_sys_Referer") = _
                                                    String.Format("{0}{1}{2}", _
                                                                    Configurations.ApplicationRoot.BrowserSystemImplementation, _
                                                                    RequestedTemplate, _
                                                                    IIf(General.Context.Request.ServerVariables.Item("QUERY_STRING") Is Nothing, String.Empty, _
                                                                        String.Format("?{0}", General.Context.Request.ServerVariables.Item("QUERY_STRING")) _
                                                                ) _
                                                            )
                                            Catch ex As Exception
                                                ' Just Handle Exceptions
                                ...
                                Case Managers.Theme.SettingsClass.ServicesClass.ServiceItem.ServiceTypes.webservice
                                    Me.WritePage(AcceptsGZip)

                            End Select
                        Else
                            Dim MethodResultText As String = Nothing
...
PARENTCALL:
                                ...
                                Else
                                    MethodResultText = SolidDevelopment.Web.PGlobals.Execution.GetPrimitiveValue(tAssembleResultInfo.MethodResult)
                                End If
                            End If

                            ' This header for examing Response.End and also sent to the browser about the name of Site Framework
                            Dim ResponseEndRaised As Boolean = False
                            Try
                                SolidDevelopment.Web.General.Context.Response.AppendHeader("X-SiteFramework", "XeoraCube")
                            ...
                            If Not ResponseEndRaised AndAlso _
                                ...
                                Select Case Me._ThemeWebControl.ServiceType
                                    Case Managers.Theme.SettingsClass.ServicesClass.ServiceItem.ServiceTypes.template
                                        ...
                                        ' Define Referer Variable
                                        If Not RequestedTemplate Is Nothing AndAlso _
                                            String.Compare(AuthenticationPage, RequestedTemplate, True) <> 0 Then

                                            Dim preRequesttedTemplate As String = Nothing
                                            Dim HTTP_REFERER As String = _
                                                SolidDevelopment.Web.General.Context.Request.ServerVariables.Item("HTTP_REFERER")
                                            Dim HTTP_REFERER_QUERY_STRING As String = Nothing

                                            If Not HTTP_REFERER Is Nothing Then
                                                If URLMI.URLMapping Then
                                                    Dim ResolvedMapped As PGlobals.URLMappingInfos.URLMappingItem.ResolvedMapped = _
                                                        PGlobals.URLMappingInfos.URLMappingItem.ResolveMappedURL(HTTP_REFERER)

                                                    If Not ResolvedMapped Is Nothing Then
                                                        preRequesttedTemplate = ResolvedMapped.TemplateID
                                                        ...
                                                        For qS As Integer = 0 To ResolvedMapped.QueryStrings.Count - 1
                                                            ...
                                                            If ResolvedMapped.QueryStrings.Count > (qS + 1) Then _
                                                                QueryString.Append("&")
                                                        Next
                                                        HTTP_REFERER_QUERY_STRING = QueryString.ToString()
                                                    Else
                                                        GoTo NOTMAPPED_HTTPREFERER
                                                    End If
                                                Else
NOTMAPPED_HTTPREFERER:
                                                    preRequesttedTemplate = SolidDevelopment.Web.General.ResolveTemplateFromURL(HTTP_REFERER)

                                                    If HTTP_REFERER.IndexOf("?") > -1 Then _
                                                        HTTP_REFERER_QUERY_STRING = HTTP_REFERER.Substring(HTTP_REFERER.IndexOf("?") + 1)
                                                End If
                                            End If
                                            ...
                                            SyncLock General.Context.Session.SyncRoot
                                                Try
                                                    SolidDevelopment.Web.General.Context.Session.Contents.Item("_sys_Referer") = _
                                                        String.Format("{0}{1}{2}", _
                                                                        Configurations.ApplicationRoot.BrowserSystemImplementation, _
                                                                        preRequesttedTemplate, _
                                                                        IIf(HTTP_REFERER_QUERY_STRING Is Nothing, String.Empty, _
                                                                            String.Format("?{0}", HTTP_REFERER_QUERY_STRING) _
                                                                    ) _
                                                                )
                                                Catch ex As Exception
                                                    ' Just Handle Exceptions
                                        ...
                                        Me.WritePage(MethodResultText, AcceptsGZip)
...
QUICKFINISH:
        ...
        Private Overloads Sub WritePage(ByVal SendAsCompressed As Boolean)
            Dim sW As New IO.StringWriter

            sW.Write("<?xml version=""1.0"" encoding=""utf-8"" ?>")

            Me._ThemeWebControl.RenderService(Me._RequestID)
            sW.Write(Me._ThemeWebControl.RenderedService)

            sW.Flush()
            sW.Close()

            Me.WriteOutput(Me._ThemeWebControl.MimeType, sW.ToString(), SendAsCompressed)
        End Sub

        Private Overloads Sub WritePage(ByVal MethodResultText As String, ByVal SendAsCompressed As Boolean)
            ' Render Page
            Me._ThemeWebControl.RenderService(Me._RequestID)
            ...
            Me.WriteOutput(Me._ThemeWebControl.MimeType, RenderedService, SendAsCompressed)
        End Sub
        ...
        Public Sub KillHandler()
            SolidDevelopment.Web.General.SetRequestHttpContext(Me._RequestID, Nothing)
        End Sub

        Private Shared _Bandwidth As Long = -1
        Private ReadOnly Property Bandwidth() As Long
            Get
                If RequestHandler._Bandwidth = -1 Then
                    ...
                    If Not String.IsNullOrEmpty(_Bandwidth) Then
                        Try
                            RequestHandler._Bandwidth = Long.Parse(_Bandwidth)
                        Catch ex As Exception
                            RequestHandler._Bandwidth = 0
                        End Try
                    End If
                End If
                
                Return RequestHandler._Bandwidth
            End Get
        End Property
    End Class
End Namespace