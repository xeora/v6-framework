...
Namespace Xeora.Web.Shared
    ...
    Public Class MimeType
        ...
        Private Shared _ExtensionMap As Generic.Dictionary(Of String, String) = Nothing
        Private Shared ReadOnly Property MimeMapping() As Generic.Dictionary(Of String, String)
            Get
                If MimeType._ExtensionMap Is Nothing Then
                    MimeType._ExtensionMap = New Generic.Dictionary(Of String, String)
                    ...
                    For Each Item As Generic.KeyValuePair(Of String, String) In Configurations.CustomMime
                        MimeType._ExtensionMap.Item(Item.Key) = Item.Value
                    Next
                End If
        ...
        Private Shared Function ResolveMime(ByVal MimeLookup As MimeLookups, ByVal SearchValue As String) As String
            Select Case MimeLookup
                Case MimeLookups.Type
                    If String.IsNullOrEmpty(SearchValue) Then Return "application/octet-stream"

                    If MimeType.MimeMapping.ContainsKey(SearchValue) Then _
                        Return MimeType.MimeMapping.Item(SearchValue)

                    Return "application/octet-stream"
                Case MimeLookups.Extention
                    If String.IsNullOrEmpty(SearchValue) Then Return ".dat"

                    If MimeType.MimeMapping.ContainsValue(SearchValue) Then
                        For Each Item As Generic.KeyValuePair(Of String, String) In MimeType.MimeMapping
                            If String.Compare(Item.Value, SearchValue) = 0 Then Return Item.Key
                        Next
                    End If
